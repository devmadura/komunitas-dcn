...
export default function KlaimSertifikatPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(`/api/sertifikat?email=${encodeURIComponent(email)}`);
    ...
    <div className="min-h-screen bg-background py-8 px-4 pt-32">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-foreground mb-2">Klaim Sertifikat</h1>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-card rounded-xl shadow-lg p-6 border border-border">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-bold text-foreground">{data.kontributor.nama}</h2>
                  <p className="text-muted-foreground">{data.kontributor.email}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-card rounded-xl shadow-lg p-6 border border-border">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-semibold text-foreground">Sertifikat Event/Pertemuan</h3>
              </div>
            ...
            <div className="bg-card rounded-xl shadow-lg p-6 border border-border">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-semibold text-foreground">Sertifikat Quiz</h3>
              </div>

              <div className="p-4 bg-muted/50 rounded-lg">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-foreground">Progress Quiz</span>
                  <span className="font-bold text-foreground">{data.quiz.total} / 5</span>
                </div>
                ...
                <div className="w-full bg-muted rounded-full h-3 mb-4">
                  <div
                    ...
                    style={{ width: `${Math.min((data.quiz.total / 5) * 100, 100)}%` }}
                  />
                </div>
                ...
                {data.quiz.eligible ? (
                  <div className="space-y-3">
                    ...
                    >
                      ...
                      {data.quiz.claimed ? "Download Ulang Sertifikat" : "Download Sertifikat Quiz"}
                    </button>
                  </div>
                ) : (
                  <div className="flex items-center justify-center gap-2 text-muted-foreground">
                    <Clock className="w-5 h-5" />
                    <span>Ikuti {5 - data.quiz.total} quiz lagi untuk unlock</span>
                  </div>
                )}
              </div>
            </div>
